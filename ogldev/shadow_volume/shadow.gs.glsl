...
{    
    ...
    gl_Position = MVPMatrix * vec4(LightDir, 0.0);
    EmitVertex();
...
void main()
{
    ...
    vec3 LightDir = normalize(lightPosition - PosL[0]);
    ...
    if (dot(Normal, LightDir) > 0) {
        ...
        // render the back cap
        LightDir = PosL[0] - lightPosition;
        gl_Position = MVPMatrix * vec4(LightDir, 0.0);
        EmitVertex();

        LightDir = PosL[4] - lightPosition;
        gl_Position = MVPMatrix * vec4(LightDir, 0.0);
        EmitVertex();

        LightDir = PosL[2] - lightPosition;
        gl_Position = MVPMatrix * vec4(LightDir, 0.0);
        EmitVertex();
